...
| Repo | State |
|---|---|
| **FotH dev** | **MIRRORED 2026-07-26:** dev `18a07f0` → prod `3b304f5` (Pages deploy verified). The mirror carried the whole 15-week batch — consolidation (`e1da2dd`), curriculum batch (`c4f92c8`), Keeping the Flame (`562fb6b`), John's four review-fix commits (`b7240da`…`9388a72`), plus two 2026-07-26 sweeps: cross-series/shared refs (`5d9f84c`) and spelled-out week counts (`18a07f0`). Manifest + search index were regenerated IN PROD with prod's tools; prod's legacy `search-index.js` deleted; standing divergences preserved (DEV banner, IJH `-dev` URLs, generator SITE config). Dev and prod are in content sync. **2026-07-26 (later):** the 5-piece safety additions (Settle Protocol, adapted on John's word from an ILP shadow-work safety protocol a friend shared) SHIPPED to dev (`9e3aeb1`) and MIRRORED to prod (`a1236fd`) — §6 Settle Protocol + nobody-leaves-activated + four-level escalation, App C card, consent-form pause/stop sentences, precohort extra-care flags, Settle lines in the six heavy weeks, FC1 Lab 4 settle drill. The new consent sentences + extra-care flags ride the same CCA/VA legal-review gate as the rest of the packet. **2026-07-29:** "Companions and Coverings" shipped + mirrored (dev `3a82af7` / prod `dc62c97`) — the training-identity page: no licensing ever, local covering is the unit of trust, build-your-own Companions, Apollos letter for outside Companions, levels = self-assigned safety floors. |
