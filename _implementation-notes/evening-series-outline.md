# Evening Series Outline — DRAFT v1

**The biweekly evening form of Getting Started, per the planning meeting with
the pilot family's parents (September 2026). Outline first; slides and
session materials change only after John approves this shape.**

## 1. The evening, fixed

| Time | What | Who leads |
|---|---|---|
| 6:00 | Dinner together. The table is part of the container. | The household |
| 7:00 | The session begins (90 minutes, blocks below) | The teen teams |
| 8:30 | Session complete. Fellowship, dessert, goodbyes. | Everyone |
| 9:00 | Done. | |

Meeting night is Monday or Tuesday of the meeting week, set by the family's
schedule. Every two weeks.

## 2. The 90-minute run sheet (adapted from the 75-minute school version)

The school version's blocks survive with their durations mostly intact; the
fifteen added minutes go where the evening needs them: a fuller check-in
(two weeks of life to report, not one) and a real commissioning of the
homework.

| Time | Block | Lead |
|---|---|---|
| 7:00-7:08 | Welcome and centering. Blessing, container reminder, frame the evening. | Container holder |
| 7:08-7:25 | Check-in circle: two weeks of practice reported, midweek check-ins debriefed. One-sentence shares, a parent opens. | Container holder |
| 7:25-7:45 | Teaching block: the session's content. | Content presenter |
| 7:45-8:10 | Practice block: the session's exercise, in circle. | Content presenter, container holder watching the room |
| 8:10-8:22 | Shared circle: what surfaced. Parents first. | Container holder |
| 8:22-8:28 | Homework commissioning: walk the handout, each person names their check-in partner for the midweek report. | Content presenter |
| 8:28-8:30 | Closing blessing. | Container holder |

A drop-in lands softly: the welcome block always includes two minutes of
"where we are on the map," and every session's content stands alone by
design.

## 3. The two roles and the rotation

- **Container holder**: opens and closes, holds the check-in and shared
  circles, watches the room during practice. Safety and warmth.
- **Content presenter**: carries the teaching and practice blocks, and
  commissions the homework.
- With four teens, the roles can be held by pairs or by individuals as the
  rotation allows; teams step up as ready and willing, and they have been.
- An adult (John or a parent) is always the backstop and never far from
  anything personal; the bright line holds as always.
- **The planning lab**: about thirty minutes by phone with John in the off
  week, walking the leading team through the session's issues, fragilities,
  and passes.

## 4. The two-week homework, with the midweek check-in

Each session's handout now has four parts:

1. **The practice** for the first week, as in the school version.
2. **The midweek check-in**: before the session's one-week mark, report to
   the person you named at commissioning on how you are doing against the
   practice. A text, a call, or a hallway conversation all count. The point
   is being expected.
3. **The practice continued** for the second week, deepened or repeated as
   the session specifies.
4. **One sentence of preparation** for the next session (read, notice, or
   bring something).

Check-in partners are chosen fresh at each commissioning so the reporting
lines move around the group.

## 5. Where we start (the branch)

- **If new teens, parents, or standalone adults join the first evening:
  Session 1**, and the veterans' experience becomes an asset in the room.
- **Otherwise: Session 5** (Knowing and Being Known), picking up where the
  prototype group left off after sessions 1-4.

The family carries the invitations: teens, parents, and standalone adults
alike, plus drop-ins for any single session. If no one else comes, the work
continues as curriculum development and leader training with full value.

## 6. The calendar ladder (Mon/Tue of each meeting week)

| # | Week of | Note |
|---|---|---|
| 1 | Sep 14 | First evening; branch decided by who is in the room |
| 2 | Sep 28 | |
| 3 | Oct 12 | |
| 4 | Oct 26 | |
| 5 | Nov 9 | |
| 6 | Nov 23 | Thanksgiving week; family's call to meet or shift |
| 7 | Dec 7 | |
| 8 | Dec 21 | Christmas week; family's call, likely skip |
| 9 | Jan 4 | |
| 10 | Jan 18 | ...continuing biweekly |

Eighteen meetings (sessions 5-22) with two holiday skips lands the arc in
mid-June 2027; all twenty-two from Session 1 runs into August. See the
decision at section 9.

## 7. What we are testing, and how it gets captured

The test is the curriculum and the leaders, not the participants.

- **After every session, a short leader debrief** (adapted from the
  companion debrief sheets), five questions: Were the instructions clear?
  Could the team use them as written? What did they change on the fly? Where
  did the room wobble? What should the materials say differently?
- **The three vital signs**, watched as always.
- **The quarterly eval** at natural marks (roughly every sixth meeting), per
  the measurement covenant and the family's consent, as before.

## 8. Covering

Living Hope's answer on oversight is expected around September 20. If Ryan
Hammond accepts, the pilot sits under him as asked. If he declines, John
proposes that Tom Fremont, the pilot family's father and an elder at the
church, hold the oversight and report his assessment to Ryan. Either way the
pilot runs covered, with the covering named to everyone in the room.

## 9. One decision John holds open

The school version's Second Runnings (sessions 14 and 19), the Float (17),
and Return II (18) exist to give practice repetition inside a weekly
cadence. The biweekly homework, with its two weeks of practice and the
midweek check-in, already provides that repetition. Folding those four into
the homework stream would bring sessions 5-22 down to fourteen meetings and
land the arc by late April 2027. Or keep all of them and let the arc breathe
into June. To be decided from experience, not in advance; nothing needs
deciding before Session 1.

## 10. After this outline is approved

1. Adapt the Session 5 materials (and Session 1, for the branch) to the
   90-minute run sheet and the four-part homework handout.
2. Build the homework handout template with the check-in partner line.
3. Then, and only then, the slides.

*Draft v1 for John's review, 9 September 2026. Nothing changes in
docs/getting-started until he approves the shape.*
